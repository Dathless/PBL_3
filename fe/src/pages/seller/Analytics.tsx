...
import { getAnalyticsSummary, getRevenueSeries, getOrderSeries, getTopSellingProducts } from "@/lib/sellerApi"
import {
  BarChart,
  Bar,
  XAxis,
...
export default function AnalyticsPage() {
  ...
  const [orderSeries, setOrderSeries] = useState<{ label: string; value: number }[]>([])
  const [topProducts, setTopProducts] = useState<Array<{ name: string; sales: number; revenue: number; imageUrl?: string }>>([])
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoading(true)
      try {
        const [summary, revSeries, ordSeries, top] = await Promise.all([
          getAnalyticsSummary(sellerId, timeRange as any),
          getRevenueSeries(sellerId, timeRange),
          getOrderSeries(sellerId, timeRange),
          getTopSellingProducts(sellerId, timeRange)
        ])

        if (mounted) {
          setAnalytics(summary)
          setRevenueSeries(revSeries)
          setOrderSeries(ordSeries)
          setTopProducts(top)
        }
      } catch (error) {
        console.error("Error loading analytics:", error)
        if (mounted) {
          // Reset on error? Or keep stale? Keeping reset logic for now.
          setAnalytics({
            revenue: { current: 0, previous: 0, change: 0 },
            orders: { current: 0, previous: 0, change: 0 },
            customers: { current: 0, previous: 0, change: 0 }
  ...
  if (loading) {
    return (
      <SellerLayout activeMenu={activeMenu} setActiveMenu={setActiveMenu}>
        <div className="flex h-screen items-center justify-center">Loading analytics...</div>
      </SellerLayout>
    )
  }
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="flex items-center gap-1 mt-2">
              <TrendingUp className="w-4 h-4 text-green-600" />
              <span className={`text-sm ${analytics.orders.change >= 0 ? "text-green-600" : "text-red-600"}`}>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="flex items-center gap-1 mt-2">
              <TrendingUp className="w-4 h-4 text-green-600" />
              <span className={`text-sm ${analytics.customers.change >= 0 ? "text-green-600" : "text-red-600"}`}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={revenueSeries}>
                  ...
                  <XAxis
                    ...
                    tick={{ fontSize: 12 }}
                    tickFormatter={(value) => {
                      // Shorten date if needed, e.g. "Oct 01"
                      if (value.length > 5 && timeRange !== '1y') return value.substring(5);
                      return value;
                    }}
                  />
                  <YAxis
                ...
                <tbody>
                  {topProducts.map((product, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition">
                      <td className="py-3 px-4 text-sm font-semibold flex items-center gap-3">
                        {product.imageUrl && (
                          <img
                            src={product.imageUrl.startsWith('http') ? product.imageUrl : `http://localhost:8080/api/files/download/${product.imageUrl}`}
                            alt={product.name}
