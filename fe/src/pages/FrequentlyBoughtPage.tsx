    ...
    useEffect(() => {
        const loadProducts = async () => {
            try {
                ...
                const topProducts = await productApi.getTopSelling(20)

                const productList: Product[] = topProducts.map(p => ({
                    id: p.id,
                    name: p.name,
                    image: p.images && p.images.length > 0 ? p.images[0].imageUrl : '/placeholder.svg',
                    // Apply discount logic if needed, but for now just price
                    price: Number(p.price) * (1 - Number(p.discount || 0) / 100),
    ...
    return (
        <main className="min-h-screen bg-white">
            ...
            <div className="max-w-7xl mx-auto px-4 py-12">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold mb-2">Frequently Bought Together</h1>
                    <p className="text-gray-600">Popular product combinations that customers love</p>
                </div>
                            ...
                            <div key={product.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-xl transition group">
                                ...
                                <div className="p-4">
                                    ...
                                    <div className="flex items-center gap-2 mb-4">
                                        <span className="text-cyan-600 font-bold text-lg">${product.price.toFixed(2)}</span>
                                    </div>
